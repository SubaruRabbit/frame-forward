...
export type OwnedEquipment = {id: string; kind: EquipmentKind; catalogItemId: string; nickname?: string | null; primary: boolean};
export type BodyLensCombination = {cameraEquipmentId: string; lensEquipmentId: string; compatible: boolean; cropModeRequired: boolean; mode: string; defaultEligible: boolean};
...
export class EquipmentModel {
  ...
  get primaryCameraId(): string | undefined { return this.owned.find(item => item.kind === 'CAMERA' && item.primary)?.id; }

  async restore(): Promise<void> {
    const [catalog, owned, combinations] = await Promise.all([this.api.catalog('CAMERA'), this.api.owned(), this.api.combinations()]);
  ...
  search(query: string): CatalogItem[] {
    ...
    return normalized ? this.catalog.filter(item => `${item.brand} ${item.model}`.toLowerCase().includes(normalized)) : this.catalog;
  }
  ...
  async setPrimary(id: string): Promise<void> {
    ...
    this.owned = this.owned.map(item => item.kind === 'CAMERA' ? {...item, primary: item.id === selected.id} : item);
  }

  incompatibilityFor(lensEquipmentId: string): string | undefined {
    const incompatible = this.combinations.find(item => item.lensEquipmentId === lensEquipmentId && !item.compatible);
