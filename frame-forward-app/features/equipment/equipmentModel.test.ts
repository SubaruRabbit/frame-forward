...
test('catalog search, add/remove, primary restore, and incompatible feedback remain coherent', async () => {
  const api = {
    ...
    owned: async () => [
      { id: 'camera-a', kind: 'CAMERA' as const, catalogItemId: 'sony-a6700', primary: true },
      { id: 'camera-b', kind: 'CAMERA' as const, catalogItemId: 'nikon-z8', primary: false },
    ],
    add: async () => ({
      id: 'lens-a',
      kind: 'LENS' as const,
      catalogItemId: 'canon-rf-24-70-l',
      primary: false,
    }),
    ...
    setPrimary: async () => ({
      ...
      catalogItemId: 'nikon-z8',
      primary: true,
    }),
    combinations: async () => [
      {
        cameraEquipmentId: 'camera-a',
        lensEquipmentId: 'lens-a',
        compatible: false,
        cropModeRequired: false,
        mode: 'CROSS_MOUNT',
