...
import {activeAiTasks} from './aiTasks';
test('a simulated restart restores terminal state and clears its active id', async () => {
  ...
  await expect(activeAiTasks.restore({get: async id => ({taskId:id,state:'FAILED',errorCode:'INVALID_MODEL_OUTPUT'})})).resolves.toEqual([{taskId:'task-1',state:'FAILED',errorCode:'INVALID_MODEL_OUTPUT'}]);
