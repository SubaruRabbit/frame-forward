...
- [x] 1.2 `frame-forward-app/shared`：补齐共享 UI 的可访问名称、语义和字号验证；验证组件测试与人工检查。
- [x] 1.3 `frame-forward-app/android`：建立受控构建与发布检查；验证 Android 构建可追溯。
- [x] 1.4 `frame-forward-app/ios`：建立 iOS 构建检查；验证 iOS 配置和构建可执行。
