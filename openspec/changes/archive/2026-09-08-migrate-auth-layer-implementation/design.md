...
建立 auth 目标分层实现，封装持久化并保留迁移期兼容入口。涉及 auth、bootstrap；不调整外部行为。集成测试发现原 @MapperScan 将新旧清理端口都当作 Mapper，产生同名 Bean 冲突；bootstrap 将扫描限定为 @Mapper 标记，所有现有持久化 Mapper 已具有该标记，新增回归断言。
