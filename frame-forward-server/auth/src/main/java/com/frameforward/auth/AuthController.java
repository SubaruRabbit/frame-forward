...
public class AuthController {
    ...
    ResponseEntity<AuthService.AccountDeletionJob> startDeletion(@RequestHeader(name = "Authorization", required = false) String authorization, @RequestBody AccountDeletionRequest request) { return ResponseEntity.status(HttpStatus.ACCEPTED).body(auth.startAccountDeletion(bearer(authorization), request.currentPassword())); }
    @GetMapping("/account-deletions/{jobId}")
    AuthService.AccountDeletionJob deletionStatus(@PathVariable String jobId, @RequestHeader(name = "X-Account-Deletion-Token", required = false) String token) { return auth.deletionStatus(jobId, token); }
    @PostMapping("/account-deletions/{jobId}")
    ResponseEntity<AuthService.AccountDeletionJob> retryDeletion(@PathVariable String jobId, @RequestHeader(name = "X-Account-Deletion-Token", required = false) String token) { return ResponseEntity.status(HttpStatus.ACCEPTED).body(auth.retryAccountDeletion(jobId, token)); }
