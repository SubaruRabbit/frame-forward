...
class UserEquipmentIntegrationTests {
    ...
    @Test void ownedEquipmentIsPrivateAndFirstCameraBecomesPrimary() throws Exception {
        var owner = register("equipmentowner");
        var other = register("equipmentother");
        var item = add(owner, "CAMERA", "sony-a6700");

        mvc.perform(get("/equipment").header("Authorization", owner))
    ...
    @Test void primarySwitchIsAtomicAndCombinationsExcludeCrossMountDefaults() throws Exception {
        ...
        mvc.perform(put("/equipment/cameras/{id}/primary", second).header("Authorization", owner))
            .andExpect(status().isOk()).andExpect(jsonPath("$.id").value(second)).andExpect(jsonPath("$.primary").value(true));
        Integer switched = jdbc.queryForObject("SELECT COUNT(*) FROM user_equipment WHERE id IN (?, ?) AND is_primary = TRUE", Integer.class, first, second);
        org.assertj.core.api.Assertions.assertThat(switched).isEqualTo(1);
        mvc.perform(get("/equipment/body-lens-combinations").header("Authorization", owner))
            .andExpect(status().isOk()).andExpect(jsonPath("$.items[?(@.cameraEquipmentId == '" + second + "' && @.lensEquipmentId == '" + canonLens + "')].compatible").value(false))
            .andExpect(jsonPath("$.items[?(@.cameraEquipmentId == '" + second + "' && @.lensEquipmentId == '" + canonLens + "')].defaultEligible").value(false));
    }
    ...
    private String add(String authorization, String kind, String catalogItemId) throws Exception {
        var response = mvc.perform(post("/equipment").header("Authorization", authorization).contentType(MediaType.APPLICATION_JSON)
