...
class PortfolioIntegrationTests {
	...
	void deletionIsIdempotentHidesTheWorkAndKeepsUnrelatedWork() throws Exception {
		...
		Assertions.assertEquals(0, jdbc.queryForObject("SELECT COUNT(*) FROM photo_evaluations WHERE media_id = ?",
				Integer.class, target));
		Assertions.assertEquals(1, jdbc.queryForObject("SELECT COUNT(*) FROM photo_evaluations WHERE media_id = ?",
