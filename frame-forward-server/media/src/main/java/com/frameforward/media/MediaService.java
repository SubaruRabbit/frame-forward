...
@Service public class MediaService {
  ...
  @Transactional public MediaResponse ingest(String ownerId, MultipartFile upload) {
    if (upload == null || upload.isEmpty()) throw new InvalidMediaException("A JPEG file is required");
    if (upload.getSize() > MAX_BYTES) throw new TooLargeException();
    Path temporary = null;
    try {
      Files.createDirectories(root.resolve("tmp")); temporary = Files.createTempFile(root.resolve("tmp"), "upload-", ".jpg");
      try (InputStream in = upload.getInputStream()) { copyBounded(in, temporary); }
      String hash = sha256(temporary); var existing = media.selectOne(new LambdaQueryWrapper<MediaEntity>().eq(MediaEntity::getOwnerId, ownerId).eq(MediaEntity::getContentHash, hash));
      if (existing != null) return response(existing);
      byte[] signature = Files.readAllBytes(temporary); if (signature.length < 3 || (signature[0]&255)!=255 || (signature[1]&255)!=216 || (signature[2]&255)!=255) throw new InvalidMediaException("File is not a JPEG");
      BufferedImage decoded = ImageIO.read(temporary.toFile()); if (decoded == null) throw new InvalidMediaException("JPEG cannot be decoded");
      Metadata metadata = ImageMetadataReader.readMetadata(temporary.toFile()); int orientation = orientation(metadata); BufferedImage corrected = orient(decoded, orientation);
      String id = UUID.randomUUID().toString(); Path originals = root.resolve("original"); Path ai = root.resolve("ai"); Files.createDirectories(originals); Files.createDirectories(ai);
      Path original = originals.resolve(id + ".jpg"); Path sanitized = ai.resolve(id + ".jpg");
      Files.move(temporary, original, StandardCopyOption.ATOMIC_MOVE); temporary = null;
      if (!ImageIO.write(corrected, "jpeg", sanitized.toFile())) throw new InvalidMediaException("JPEG encoder unavailable");
      MediaEntity saved = new MediaEntity(id, ownerId, hash, corrected.getWidth(), corrected.getHeight(), original.toString(), sanitized.toString(), allowedExif(metadata)); media.insert(saved); return response(saved);
    } catch (TooLargeException|InvalidMediaException e) { throw e;
    } catch (Exception e) { throw new InvalidMediaException("JPEG could not be processed");
  ...
  public static class InvalidMediaException extends RuntimeException { InvalidMediaException(String message){super(message);} } public static class TooLargeException extends RuntimeException {} public static class NotFoundException extends RuntimeException {}
}
