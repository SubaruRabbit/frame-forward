...
class CourseGenerationValidatorTest {
	...
	void rejectsMissingSchemaDuplicateLessonsAndInventedMenus() {
		...
		assertFalse(CourseGenerationValidator.valid(Map.of("title", "x", "lessons",
				List.of(Map.of("id", "a", "objective", "x"), Map.of("id", "a", "objective", "x")))));
		assertFalse(CourseGenerationValidator.valid(
				Map.of("title", "x", "lessons", List.of(Map.of("id", "a", "objective", "x", "menuPath", "未验证")))));
	}
