...
class AiTaskRuntimeTest {
	...
	void returnsExistingTaskAfterIdempotencyConflictWithoutRescheduling() {
		...
		doThrow(new DuplicateKeyException("duplicate")).when(tasks).insert(any(AiTaskEntity.class));
	...
	private static Fixture runtimeFixture(AiTaskMapper tasks, AuthService auth) {
		...
		AiTaskBusiness business = new AiTaskBusiness(new AiTaskManager(new AiTaskRepository(tasks), List.of()),
	...
	private record Fixture(
			...
			SceneAnalysisGraph sceneGraph,
			PlanGenerationGraph planGraph,
			PhotoEvaluationGraph evaluationGraph,
