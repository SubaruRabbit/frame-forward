...
public class AiTaskRuntime {
  ...
  private final AiTaskMapper tasks; private final ObjectMapper json; private final AuthService auth; private final Executor executor;
  private final String defaultModel, coachModel; private final Map<String, List<SseEmitter>> subscribers = new ConcurrentHashMap<>();
  public AiTaskRuntime(AiTaskMapper tasks, ObjectMapper json, AuthService auth, @Qualifier("applicationTaskExecutor") Executor executor,
      ...
      @Value("${frame-forward.ai.qwen-routes.coach:qwen-plus}") String coachModel) { this.tasks=tasks; this.json=json; this.auth=auth; this.executor=executor; this.defaultModel=defaultModel; this.coachModel=coachModel; }
  ...
  public Created create(String accessToken, String key, CreateRequest request) {
    ...
    executor.execute(() -> run(task.id)); return new Created(task.id, State.QUEUED);
  }
  ...
  private Map<String,Object> outputFor(AiTaskEntity task, int attempt) { Map<String,Object> input=read(task.inputJson); Object supplied=input.get("mockOutput"); if (supplied instanceof Map<?,?> map) return (Map<String,Object>)map; if ("course-generation".equals(task.operationType)) return Map.of("title","P0 course","lessons",List.of(Map.of("id","lesson-1","objective","掌握一个可练习的摄影目标")),"result","accepted","modelId",task.modelId); return Map.of("result", "accepted", "modelId", task.modelId); }
  private static boolean valid(Map<String,Object> output) { return output != null && output.get("result") instanceof String value && !value.isBlank() && (!output.containsKey("lessons") || CourseGenerationValidator.valid(output)); }
  ...
  private String modelFor(String operation) { return "course-generation".equals(operation) ? "qwen3.7-plus" : "coach".equals(operation) || "course-feedback".equals(operation) ? coachModel : defaultModel; }
