...
public class AiTaskRuntime {
    ...
    private final String defaultModel, coachModel, sceneModel, imageModel;
    private final Map<String, List<SseEmitter>> subscribers = new ConcurrentHashMap<>();
    public AiTaskRuntime(AiTaskMapper tasks, ObjectMapper json, AuthService auth,
    ...
    public void recoverInterruptedTasks() {
        for (var task : tasks
                .selectList(new LambdaQueryWrapper<AiTaskEntity>().eq(AiTaskEntity::getState, State.RUNNING.name()))) {
            task.state = State.QUEUED.name();
    ...
    public SseEmitter events(String accessToken, String id) {
        ...
        emit(emitter, status(task));
        return emitter;
    }
    void run(String id) {
        var task = tasks.selectById(id);
        if (task == null || !transition(task, State.QUEUED, State.RUNNING))
            return;
        try {
            for (int attempt = 0; attempt < 3; attempt++) {
                Map<String, Object> output = outputFor(task, attempt);
                if (valid(task.operationType, output)) {
                    ...
                    task.errorCode = null;
                    transition(task, State.RUNNING, State.SUCCEEDED);
                    return;
                }
            }
            task.resultJson = null;
            task.errorCode = "INVALID_MODEL_OUTPUT";
            transition(task, State.RUNNING, State.FAILED);
        } catch (RuntimeException exception) {
            log.warn("AI task {} failed during {}", task.id, task.operationType, exception);
            task.resultJson = null;
            task.errorCode = "RUNTIME_ERROR";
            transition(task, State.RUNNING, State.FAILED);
        }
    }
    private boolean transition(AiTaskEntity task, State from, State to) {
        if (State.valueOf(task.state) != from || !legal(from, to))
            return false;
        task.state = to.name();
        task.updatedAt = Instant.now();
        tasks.updateById(task);
    ...
    static boolean legal(State from, State to) {
        return (from == State.QUEUED && (to == State.RUNNING || to == State.CANCELLED))
                || (from == State.RUNNING && (to == State.SUCCEEDED || to == State.FAILED || to == State.CANCELLED));
    }
    private Map<String, Object> outputFor(AiTaskEntity task, int attempt) {
    ...
    private AiTaskEntity owned(String token, String id) {
        var task = tasks.selectById(id);
        if (task == null || !task.accountId.equals(auth.requireAccountId(token)))
            throw new NotFound();
        return task;
    }
    private Status status(AiTaskEntity task) {
        return new Status(task.id, State.valueOf(task.state), task.resultJson == null ? null : read(task.resultJson),
                task.errorCode, new Trace(task.workflowVersion, task.modelId, task.promptVersion, task.ruleVersion,
                        task.schemaVersion));
    }
    private void publish(Status status) {
        for (var emitter : subscribers.getOrDefault(status.taskId, List.of()))
            emit(emitter, status);
    }
