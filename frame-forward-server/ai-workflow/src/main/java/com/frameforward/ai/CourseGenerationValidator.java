...
public final class CourseGenerationValidator {
    ...
    public static boolean valid(Map<String, Object> output) {
        if (output == null || !(output.get("title") instanceof String title) || title.isBlank()
        ...
        for (Object item : lessons) {
            ...
            if (!(lesson.get("id") instanceof String id) || id.isBlank() || !ids.add(id)
                    || !(lesson.get("objective") instanceof String objective) || objective.isBlank())
                return false;
            if (lesson.containsKey("menuPath")
                    && !(lesson.get("menuPath") instanceof String path && path.contains("官方")))
                return false;
        }
        return true;
    }
