...
class EquipmentServiceBoundaryTest {
	...
	static void initializeMapping() {
		TableInfoHelper.initTableInfo(new MapperBuilderAssistant(new MybatisConfiguration(), "equipment-test"),
	...
	void primarySwitchRequiresOwnedCameraAndClearsBeforeSetting() {
		...
		var updates = ArgumentCaptor.forClass(LambdaUpdateWrapper.class);
    ...
    void removalDistinguishesMissingAndOwnedItems() {
        when(owned.delete(any(Wrapper.class))).thenReturn(0, 1);
