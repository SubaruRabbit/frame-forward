...
@Component
class ReferenceImageBusiness {
    private final ReferenceImageManager manager;

    ReferenceImageBusiness(ReferenceImageManager manager) {
        this.manager = manager;
    }

    void validate(ReferenceImageService.Request request) {
        if (!hasRequiredContext(request) || !hasRequiredPlanText(request.selectedPlan)
                || !hasStructuredPlanFields(request.selectedPlan)) {
            throw new ReferenceImageService.InvalidRequest();
        }
    }
    ...
    String controlledPrompt(Map<String, Object> selectedPlan) {
    ...
    void persistReference(String accountId, String environmentMediaId, String shootingPlanId, String taskId,
            Map<String, Object> selectedPlan, String prompt) {
        manager.persistReferenceIfAbsent(new ReferenceImageManager.NewReference(accountId, environmentMediaId,
                shootingPlanId, taskId, String.valueOf(selectedPlan.get("label")), prompt));
    }

    private static boolean hasRequiredContext(ReferenceImageService.Request request) {
