...
public class ReferenceImageController {
  ...
  public ReferenceImageController(ReferenceImageService referenceImages) { this.referenceImages = referenceImages; }
  @PostMapping
  ResponseEntity<AiTaskRuntime.Created> create(@RequestHeader(name = "Authorization", required = false) String authorization,
      @RequestHeader("Idempotency-Key") String key, @RequestBody ReferenceImageService.Request request) {
    return ResponseEntity.accepted().body(referenceImages.create(AuthController.bearer(authorization), key, request));
  }
}
